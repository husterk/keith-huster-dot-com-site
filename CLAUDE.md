...
Merging to `main` builds, deploys the Worker, then syncs `RESEND_API_KEY` and `TURNSTILE_SECRET_KEY` from 1Password (after the deploy on purpose: Cloudflare refuses secret edits while a newer preview version exists). PRs from this repo get a preview URL comment. Preview (version) URLs sit behind Cloudflare Access: a 200 can be the Access sign-in page, so check the page title. To fetch one, send `CF-Access-Client-Id` and `CF-Access-Client-Secret` from the 1Password item `Cloudflare Access preview token`, never printing them. Cloudflare cannot delete single versions, so anything ever committed stays reachable to someone who can pass Access. The custom domain is bound in `wrangler.jsonc`; `www` redirects through a Bulk Redirect in the Cloudflare dashboard.
