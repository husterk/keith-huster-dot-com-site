...
2. **Résumé.** "View résumé" (hero) and the "Résumé" nav link open `/resume`, which has a Download PDF button for `/Keith-Huster-Resume.pdf`. The build generates that file from `src/content/resume.yaml`; keep the filename stable so it can be linked from LinkedIn.
