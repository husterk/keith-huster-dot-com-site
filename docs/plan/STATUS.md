...
- PR #23 (M1): Astro 7 skeleton, Cloudflare adapter, mise-pinned Bun/Node, Base layout with fonts and tokens, placeholder pages, `ci.yml` and `deploy.yml`, Playwright smoke test. First merge deploys to `keithhuster-com.<account>.workers.dev`.
- Required status check `ci` on the `main` ruleset: add after the first CI run completes on PR #23.
- Lighthouse CI budget and the axe scan: land with M2 alongside the real homepage.
...
- Ruleset `main`: PR required, linear history, no force push or deletion.
