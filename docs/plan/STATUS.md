...
- The Turnstile script loads only when the contact form nears the viewport; it was the only third-party request on first load. It is rendered explicitly and shows its error code with a Retry button when it fails.
