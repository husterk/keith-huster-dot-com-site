...
- Merged 2026-09-26: pages crossfade in 250ms through CSS cross-document view transitions with the header held in place, off under reduced motion (#159); every page ends with the homepage footer from `src/components/Footer.astro`, which marks the current page with aria-current (#161); /colophon scrolls as one page, with the route map beside "The route, section by section", a one-line description per route stop, the stats 2x2 beside the intro, two-column decisions, and a credits strip of 15 tool logos that link to their sites (#163).
- Lighthouse in CI: median of three runs against 95/100/95/100 on /, /colophon, /resume and /404. Playwright: 61 tests at 1440/834/390 including the rider-visible assertion, the phone menu, axe on four pages, the generated PDF (two tagged pages, size, links, no phone number), the résumé facts, the contact endpoint (405, 403, 400, honeypot, dry run, 429, no-JS HTML), rider motion under both motion preferences, and the eased hash scroll.
