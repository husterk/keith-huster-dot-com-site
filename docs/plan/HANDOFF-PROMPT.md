...
**How to use it:** open a terminal in `~/git-repos/keith-huster-dot-com-site`, start `claude`, and paste everything below the line as the first message. Before that, finish the M0 manual tasks in `docs/plan/MANUAL-SETUP.md` (§3–§7) and make sure `gh auth status` works with the `workflow` scope (`gh auth refresh -s workflow`), since the session will push `.github/workflows/`.
...
- **This repository is public. No secrets, tokens, keys, account IDs, personal phone numbers or private information may ever be committed, printed in an issue/PR, or echoed to a log.** Secrets live in 1Password; the only GitHub secret is `OP_SERVICE_ACCOUNT_TOKEN`; runtime secrets reach the Worker via `wrangler secret bulk` in the deploy workflow. `.dev.vars`, `.env*` and `.secrets/` are gitignored; keep them that way. Before every commit, scan the diff for anything secret-shaped. The public values are the Turnstile site key and the Web Analytics beacon token; those are allowed in `src/content/site.yaml`.
...
4. Then start **M1** per `06-milestones.md`, in a branch, as a PR: Astro 7 skeleton with the Cloudflare adapter, `wrangler.jsonc` and `renovate.json` from samples, `.bun-version`, `.nvmrc`, Prettier, `Base.astro` with fonts and tokens, placeholder `index.astro` and `404.astro`, `ci.yml` and `deploy.yml`. The first real deploy needs the Cloudflare token in 1Password and `OP_SERVICE_ACCOUNT_TOKEN` in GitHub (MANUAL-SETUP §3–§4); if those aren't done yet, get everything else green in CI and leave the deploy job for me to trigger.
...
- [ ] §7 Web Analytics site created; **beacon token:** `________________`
- Old site hosting: `________________` (where keithhuster.com is served today, so we can plan the cutover)
